...
void InitializeVertexBuffer()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertexPositions), vertexPositions, GL_STREAM_DRAW);  // GL_STREAM_DRAW means updat position continually.  GL_STATIC_DRAW means only update position once.
	glBindBuffer(GL_ARRAY_BUFFER, 0);														  // ^ These are "Hints," and they are used to improve performance.
}
...
void AdjustVertexData(float fXOffset, float fYOffset)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, positionBufferObject);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertexPositions), &fNewData[0]);  // glBufferSubData only substitutes data into the buffer object.
	glBindBuffer(GL_ARRAY_BUFFER, 0);                                            // ^ It will not allocate new memory.
}
